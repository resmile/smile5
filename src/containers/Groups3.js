...
function Table({
  ...
  const multiSelect = filterBuyer.map((a) => { return {label:a.label, value:a.label}})
...
const groupMemEditor = memo(forwardRef((props, ref) => {
    ...
    if(props.value!=null){
      ...
      selectedVConvertObj = preSelectedVConvertObj.map((a) => { return {label:a, value:a}})  
    }
    ...
    const handleChange = (e) => { 
     ...
     let dropdownData1=e.map(d => d.label);
...
<Select
        defaultValue={selectedVConvertObj}
